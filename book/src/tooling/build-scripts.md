...
The `Build.Package` value is opaque in v0.63. It is passed by the build driver, but scripts cannot inspect package fields. Package-aware and standard workflow steps use only package metadata already known to the driver.
...
The standard workflow intentionally does not include an executable step in v0.63. Add `Build.executable for package.` explicitly when a package needs one.
...
The v0.63 Build API records standard workflows, named release/clean/format/validation/test steps, package-aware artifact/documentation steps, aggregate groups, a default step, and older named artifact/documentation forms:
