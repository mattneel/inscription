...
The `Build.Package` value is opaque in v0.65. It is passed by the build driver, but scripts cannot inspect package fields. Package-aware and standard workflow steps use only package metadata already known to the driver.
...
The standard workflow intentionally does not include an executable step in v0.65. Add `Build.executable for package.` explicitly when a package needs one.
...
The v0.65 Build API records standard workflows, named release/clean/format/validation/test steps, package-aware artifact/documentation steps, aggregate groups, a default step, and older named artifact/documentation forms:
...
Build.release archive package named "archive-dist".
Build.clean package.
...
Build scripts can request the default package release bundle:
...
A common pattern is to keep the standard CI/release groups and add an explicit bundle step:

```inscription,no-check
Build.standard package workflow.
Build.release package.
Build.release archive package.
Build.group named "dist" with steps "release" and "bundle" and "archive".
```
