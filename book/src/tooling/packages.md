...
Use `inscription build path/to/package --list` to list the script's steps and default, `inscription build path/to/package ci` to run a group, `inscription build path/to/package library` to build one artifact step, or `inscription build path/to/package` to run the declared default step. v0.54 build scripts call only the built-in `Build` API for package check/test/build/group/book requests and package-aware defaults; they cannot run shell commands, access arbitrary files, import package source modules, or customize output paths.
...
Remote dependencies, registries, lockfiles, version solving, target triples, build profiles, custom output paths, arbitrary filesystem/process/network access, and general build graph scripting remain future work. See [Build Scripts](build-scripts.md) for the v0.54 `build.ins` surface.
