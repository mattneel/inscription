...
Use `inscription build path/to/package --list` to list the script's expanded steps and default, `inscription build path/to/package release` to run the standard release group, `inscription build path/to/package library` to build one artifact step, or `inscription build path/to/package` to run the declared default step. v0.63 build scripts call only the built-in `Build` API for standard workflows, package release/clean/format/check/test/build/group/book requests, and package-aware defaults; they cannot run shell commands, access arbitrary files, import package source modules, or customize output paths.
