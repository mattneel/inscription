...
This repository currently implements **Inscription v0.65: source symbol indexes**. v0.65 adds deterministic `inscription symbols SOURCE` and `inscription package symbols ROOT` JSON indexes for source declarations, package/build declarations, and references. v0.64 structured JSON diagnostics, v0.63 stable diagnostic codes, the local diagnostic catalog, and `inscription explain CODE` / `inscription explain --list` remain available. v0.62 centralized diagnostic rendering with file/line/column locations and caret excerpts, v0.61 version reporting and doctor, plus v0.60 deterministic release archives/checksums, remain available. v0.59 release bundles generate deterministic integration directories containing static libraries, headers, interface JSON, metadata, and optional executables/docs. v0.58 package clean, v0.57 package-wide formatting, v0.56 package init/new skeleton generation for formatter-clean `package.ins`, `build.ins`, source, tests, and optional mdBook docs remains available. The existing narrow interpreted `build.ins` package build script, `Build.standard package workflow.` shortcut, sandboxed mdBook steps, named groups, defaults, package release/clean/format/check/test steps, artifact requests, v0.49 `comptime` scalar evaluation, v0.48 interpreter groundwork, v0.47 package path dependencies, package-aware static libraries, executables, LLVM IR, interface JSON, and C headers, first-class source tests, comments, documentation comments, owned buffer literal/copy initialization, pattern alternatives, integer ranges, match guards, exhaustive matches, and move-aware owned-buffer control flow remain available. The mdBook documentation site remains the primary language guide.
...
- modules, imports, package-aware module roots, local path dependencies, package skeleton generation, package-wide formatting, safe package clean, package release bundles and archives/checksums, package health diagnostics, source-span diagnostics, stable diagnostic codes, structured JSON diagnostics, source symbol/reference indexes, package build artifact routing, and `build.ins` standard workflow shortcuts plus named release/clean/format/artifact/check/test/group/book steps
...
Explain a diagnostic code and request machine-readable diagnostics:

```sh
PYTHONPATH=src python -m inscription explain INS-SEM-0001
PYTHONPATH=src python -m inscription explain --list
PYTHONPATH=src python -m inscription compile /tmp/bad.ins --diagnostic-format json
...
PYTHONPATH=src python -m inscription package clean /tmp/hello-inscription --dry-run
PYTHONPATH=src python -m inscription package release /tmp/hello-inscription --dry-run
PYTHONPATH=src python -m inscription package release /tmp/hello-inscription --archive --checksum --clean
PYTHONPATH=src python -m inscription package symbols /tmp/hello-inscription --pretty
...
Package manifests live in `package.ins`. They are declarative metadata, not executable build scripts: package metadata, source/test directory layout, a root module, exposed module validation, local path dependencies, and package-aware artifact builds stay parse-only. v0.65 adds package-aware source symbol/reference indexes; v0.64 adds structured JSON diagnostics; v0.63 adds stable diagnostic codes and `inscription explain`; v0.62 adds source-span diagnostic excerpts; v0.61 added version/doctor health reporting and release compiler metadata; v0.60 adds deterministic release archives and checksums on top of v0.59 release bundles, v0.58 safe package clean, v0.57 package-wide formatting, and v0.56 `package init`/`package new` skeleton generation and optional `build.ins` interpreted build scripts with standard package workflow shortcuts, package-aware artifact/documentation defaults, mdBook documentation steps, named groups, defaults, package release/clean/format/check/test workflow steps, and standard artifacts, while remote dependencies, registries, lockfiles, version solvers, arbitrary filesystem/process APIs, and custom build graph scripting remain out of scope.
...
PYTHONPATH=src python -m inscription package init . --name ExistingDirectoryPkg
```

Generated packages include `package.ins`, `build.ins`, a root source module, and `tests/basic.ins`; `--with-book` also creates a minimal mdBook skeleton. The generated `build.ins` uses `Build.standard package workflow.`.
...
Run `inscription package format --check` to verify canonical formatting for `package.ins`, `build.ins`, package sources, and tests; use `--in-place` to rewrite them. Add `--include-dependencies` to format local path dependencies too, and `--include-book` to run package book example checks. Run `inscription package clean` to remove only the package `build/` directory; use `--dry-run` to preview and `--include-dependencies` to clean local path dependency package build directories too. Run `inscription package check` to validate the manifest, source layout, and dependency graph. Run `inscription package test` to discover `.ins` test files under the manifest's test directory using the package source directory and direct dependency exposed modules for imports; add `--include-dependencies` to run dependency package tests. Run `inscription package build` to emit package artifacts; the default artifact is `build/lib<Package>.a`, and root package headers intentionally omit dependency exports. Run `inscription package release` to assemble a deterministic release directory with `package.ins`, `release.json`, `interface.json`, `include/<Package>.h`, `lib/lib<Package>.a`, and optional `bin/<Package>` or `docs/`; add `--archive` for a reproducible `.tar.gz` and `--checksum` for SHA-256 manifests. `release.json` includes deterministic Inscription tool/language metadata.

Optional build scripts live in `build.ins`. They are interpreted build logic, not declarative package metadata. v0.65 requires `Import Build.` and a does phrase named `build package` that takes an opaque `Build.Package` parameter. Build API calls record named package release/archive, clean, format, validation, test, artifact, documentation, and group steps; the driver then dispatches them through existing package release/clean/format/check/test/build machinery with deterministic group dependencies.
...
Run `inscription build path/to/package --list` to list expanded steps and the default, `inscription build path/to/package release` to run the standard release group, `inscription build path/to/package library` to build one artifact step, or `inscription build path/to/package` to run the declared default step. If no default is declared, a bare build preserves source-order ordinary step execution and skips groups unless requested. Step names are simple names, not paths; outputs go under `build/`, with package-aware artifact forms using the package final name for files such as `build/libProtocolTools.a`; clean steps remove only that generated `build/` tree and refuse symlinked `build` paths; release steps create deterministic bundle directories under `build/release/`, and archive steps create reproducible `.tar.gz` plus SHA-256 sidecars. Build scripts cannot import package modules, call externs, spawn arbitrary processes, read arbitrary files, perform arbitrary I/O, deploy docs, expose arbitrary package metadata, or define custom output paths in v0.65.

Use `inscription symbols SOURCE` or `inscription package symbols ROOT` when tooling needs deterministic JSON declarations and references for navigation, docs, or package inspection. Pass `--diagnostic-format json` to compiler, formatter, package, build, release, and test commands when tooling needs machine-readable errors. JSON diagnostics are emitted on stderr, use stable diagnostic codes where available, omit source excerpts, and leave successful artifact output unchanged.

Use `inscription doctor [PACKAGE_ROOT]` when diagnosing an environment or package. `doctor --json` is stable for automation, `--require-object`, `--require-executable`, `--require-static-library`, and `--require-book` promote optional tools to required checks, and `--check-pages-workflow` verifies the local mdBook Pages workflow files without contacting GitHub.
...
- [`docs/inscription-v0.65-spec.md`](docs/inscription-v0.65-spec.md): current language sprint spec
- [`grammar/inscription-v0.65.ebnf`](grammar/inscription-v0.65.ebnf): current grammar mirror
