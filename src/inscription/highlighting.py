    ...
    class InscriptionLexer(RegexLexer):
        ...
        tokens = {
            "root": [
                ...
                (r"\b(Module|Import|Package|Version|Sources|Tests|Root|Expose|Depend|Type|Constant|Check|Record|Layout|Packed|Enum|Union|External|To|Test|Let|Require|Expect|Give|When|Otherwise|While|For|Match|be|has|backed|on|path|by|giving|exported|as|from|for|of|with|and|into|at|each|index|up|to|otherwise|anything|zero|buffer|array|view|filled|containing|copied|does|gives|length|record|layout|packed|size|alignment|offset|in|read|write|constant|check|require|module|import|extern|export|enum|union|match|type|byte|bytes|owned|then|move|when|through|ignored|comptime|Build|clean|release|archive|checksum|bundle|format|static|library|executable|header|interface|json|llvm|ir|object|lowered|named|including|dependencies|group|steps|default|step|book|checked|package|standard|workflow)\b", Keyword),
